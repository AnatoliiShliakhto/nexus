...
/// A thread-safe, type-safe configuration container using the **Typestate** pattern.
///
/// `Config` serves as the central hub for the Nexus configuration system. It manages
/// layered application settings, environment variables, and sensitive secrets within
/// a unified, `Arc`-wrapped state.
///
/// ### Example
/// ```rust,ignore
...
///     .await?
///     .fetch_vault("kv/data/production")
///     .await?;
///
    ...
    C: ConfigState,
{
    ...
    /// * `VAULT_URL`: The URL of the Vault server (Required).
