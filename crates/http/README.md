...
async fn proxy_handler(req: Request) -> Result<impl IntoResponse, ProxyRequestError> {
    ...
    spin_sdk::http::send(outbound_req).map(IntoResponse::into_response).map_err(ProxyRequestError::from)
}
```
