...
```rust,ignore
#[nx_http::spin_service]
async fn handle_request(req: Request, state: AppState) -> Result<impl Into Response, MyError> {
    nx_http::router!(
        req,
        // Static route
        ...
        get  ["user", id] => get_user_handler(id, &state).await,
        
        // Tail matching (rest: &[&str])
        get  ["static", ..rest] => serve_assets(rest).await,
