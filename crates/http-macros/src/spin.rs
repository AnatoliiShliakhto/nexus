            ...
            impl Guest for self::Spin {
                async fn handle(request: IncomingRequest) -> Result<OutgoingResponse, ErrorCode> {
                    ...
                    let span = ::nx_http::tracing::info_span!(
                        ...
                        "http.status_code" = ::nx_http::tracing::field::Empty,
                    ...
                    async move {
                        match super::#name(request).await {
                            Ok(response) => {
                                ...
                                let status = if let Ok(r) = &res { r.get_status_code() } else { 500 };
                                ::nx_http::tracing::Span::current().record("http.status_code", status as u16);
                                res
                            }
                            Err(e) => {
                                ::nx_http::tracing::Span::current().record("http.status_code", e.status().as_u16());
