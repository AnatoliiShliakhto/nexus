    ...
    async fn apply_migration(&self, migration: &Migration) -> Result<(), AppError> {
        let query = if migration.bootstrap {
            format!(
                "BEGIN TRANSACTION;
                {}
                fn::ensure_component($component, $name, $description, $status, $target, $route, $protected);
        ...
        let _ = self
            ...
            .bind(("protected", migration.protected))
            .bind(("status", if migration.system { "SYSTEM" } else { "ACTIVE" }))
            .bind(("target", "http://localhost:3000")) //todo: from ENV or args
            .bind(("route", migration.route))
            .await
            .with_help_fn(|| {
                format!(
