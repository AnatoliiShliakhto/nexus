...
use crate::services::utils::{
    find_crate_in_metadata, get_packages_in_dir, get_project_root,
};
    ...
    for node in nodes {
        for file in &node.files {
            ...
            writeln!(
                w,
                "            restricted: {},",
                node.config.access == ServiceAccess::Private
            )?;
            writeln!(w, "        }},")?;
        }
    }
...
fn add_variable_with_default(
    ...
    let variables =
        doc.entry("variables").or_insert(Item::Table(Table::new())).as_table_mut().ok_or_else(
            || AppError::codegen().with_details_fn(|| format!("{key}: `variables` root key must be a table")),
        )?;
...
fn add_http_trigger(doc: &mut DocumentMut, node: &ServiceNode) -> Result<(), AppError> {
    ...
    if node.config.access == ServiceAccess::Inner {
        ...
        if let Some(route) = node.config.route.as_deref() {
            trigger.insert("route", Item::Value(Value::from(route)));
        } else {
            return Err(AppError::codegen().with_details_fn(|| format!("{}: `route` path is missing", node.key)));
        }
    }
    ...
    let http_array = trigger_root
        .as_table_mut()
        .ok_or_else(|| AppError::codegen().with_details_fn(|| format!("{}: `trigger` must be a table", node.key)))?
        .entry("http")
        .or_insert(Item::ArrayOfTables(ArrayOfTables::new()))
        .as_array_of_tables_mut()
        .ok_or_else(|| {
            AppError::codegen().with_details_fn(|| format!("{}: `trigger.http` must be an array of tables", node.key))
        })?;
